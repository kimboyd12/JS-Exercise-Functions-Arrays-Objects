...
function getModelYears(arr) {
  const modelYear = [] // creates an empty array
  for (let i=0; i < arr.length; i++) { // loop thru one arr at a time
    modelYear.push(arr[i].car_year) // push adds the cars year to the array we created
  }
  return modelYear;
}
